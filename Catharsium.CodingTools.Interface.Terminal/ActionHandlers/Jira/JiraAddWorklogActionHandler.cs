...
namespace Catharsium.CodingTools.Interface.Terminal.ActionHandlers.Jira
{
    public class JiraAddWorklogActionHandler : BaseActionHandler, IJiraActionHandler
    {
        ...
        public async Task Run(IssueAdapter issue)
        {
            ...
            var worklogs = (await this.jira.Issues.GetWorklogsAsync(issue.InternalIssue.Key.Value)).Select(l => new WorklogAdapter(l, issue));
            ...
            await issue.InternalIssue.AddWorklogAsync(new Worklog(timespent, startDate));
        }
    }
}